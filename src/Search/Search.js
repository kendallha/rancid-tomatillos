    ...
    return (
      <form className='form'>
        ...
        <Link to={`/search/${this.state.searchInput}`}>
            <i role='button' aria-label='Search Button' className='fas fa-search fa-2x' type='submit' name='search' onClick={e => this.props.handleSearch(this.state.searchInput)}></i>
        </Link>
      </form>
    )
  }
